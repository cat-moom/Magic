...
class Gene2VecPositionalEmbedding(nn.Module):
    def __init__(self, dim, max_seq_len):
        super().__init__()
        gene2vec_weight = np.load('./data1/gene2vec_16906.npy')
...
class OutPut(nn.Module):
    def __init__(self, dropout=0.5, h_dim=2048, mid_dim=1024, out_dim=512):
        super(OutPut, self).__init__()

        self.model = nn.Sequential(
            # 卷积层
            nn.Conv2d(in_channels=1, out_channels=1, kernel_size=(7, 5), padding=(0, 2)),
            nn.ReLU(),
            # 池化层
            ...
            # 全连接层1
            nn.Linear(in_features=1 * 1994 * 3, out_features=h_dim),
            nn.ReLU(),
            nn.Dropout(dropout),

            # 全连接层2
            nn.Linear(in_features=h_dim, out_features=mid_dim),
            nn.ReLU(),
            nn.Dropout(dropout),

            # 全连接层3
            nn.Linear(in_features=mid_dim, out_features=out_dim),
            nn.ReLU(),
            nn.Dropout(dropout)
        )
    ...
    def forward(self, x):
        # 调整输入形状为 [batch, 1, 2000, 7] 以适应 Conv2d
        x = x[:, None, :, :]
        # 使用 nn.Sequential 定义的模型进行前向传播
        return self.model(x)
