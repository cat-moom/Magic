...
def fit_one_epoch(model_train, model, loss_history, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    for iteration, batch in enumerate(gen):
        ...
        if not fp16:
            ...
            logits_per_image_gene, loss_gtm = model_train(images, genes)
            logits_per_gene_image = logits_per_image_gene.t()
            labels = torch.arange(len(logits_per_image_gene)).long().to(images.device)

            loss_logits_per_image_gene               = nn.CrossEntropyLoss()(logits_per_image_gene, labels)
            loss_logits_per_gene_image                = nn.CrossEntropyLoss()(logits_per_gene_image, labels)
            loss                                = (loss_logits_per_image_gene + loss_logits_per_gene_image) / 2
            loss = loss + loss_gtm
...
'''logits_per_image_gene, logits_per_text_gene                 = model_train(images, texts, genes)
            labels                              = torch.arange(len(logits_per_image_gene)).long().to(images.device)
            logits_per_gene_image = logits_per_image_gene.t()
            logits_per_gene_text = logits_per_text_gene.t()

            loss_logits_per_image_gene               = nn.CrossEntropyLoss()(logits_per_image_gene, labels)
            loss_logits_per_gene_image = nn.CrossEntropyLoss()(logits_per_gene_image, labels)
            loss_logits_per_text_gene                = nn.CrossEntropyLoss()(logits_per_text_gene, labels)
            loss_logits_per_gene_text = nn.CrossEntropyLoss()(logits_per_gene_text, labels)
            loss                                = loss_logits_per_image_gene + loss_logits_per_text_gene +loss_logits_per_gene_image + loss_logits_per_gene_text
'''
