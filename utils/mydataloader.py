...
#         adata = self.adata[gene_path]
        # 获取h5ad文件
        if gene_path not in self.adata:
            self.adata[gene_path] = ad.read_h5ad(gene_path + '/filtered_feature_bc_matrix_one.h5ad')
        adata = self.adata[gene_path]
...
def dataset_collate(batch):
    ...
    captions = []
    genes = []
    for image, caption, gene in batch:
        images.append(image)
