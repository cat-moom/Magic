from momentum_test import MYtest_momentum as MYtest
...
adata_path = './data/Breast_cancer/GSE210616-4-1'
save_path = './data/Breast_cancer/GSE210616-4-1/'
data_list = []
...
for i in range(len(adata_list)):
    label = adata_list[i]
    data = get_near_genelabel(adata, save_path=save_path, label=label, radius=15, genenumber=9)
    data_list.append(data)


model = MYtest()
# 这个h5ad文件路径需要修改
adata = ad.read_h5ad('./data/Breast_cancer/GSE210616-4-1/filtered_feature_bc_matrix_one.h5ad')
data_len = len(data_list)
count = 0
for i in tqdm(data_list):
    # 图像保存路径需要修改
    with torch.no_grad():
        image_path = './data/Breast_cancer_image_process/GSE210616-4-1/' + i['true_label'] + '.jpeg'
        image = Image.open(image_path)

        gene_label_list = i["false_label"]
        gene_label_list.insert(0, i['true_label'])

        probs, score, max_index = model.image_detect_gene(image=image, gene_label_list=gene_label_list, adata=adata)

        index = int(np.argmax(probs, axis=1)[0])
        max_index = torch.argmax(max_index)
        if gene_label_list[index] == i['true_label'] or max_index == 0 :
            count = count + 1
